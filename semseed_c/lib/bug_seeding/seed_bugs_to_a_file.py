            ...
            if bug_seeding.is_matching_token_sequence():
                ...
                if len(mutated_token_sequences) > 0:
                    ...
                    for ms, mutated_sequence in enumerate(mutated_token_sequences):
                        ...
                        target_range = bug_seeding.get_target_locations()
                        ...
                        out_file_name = file_name.replace(
                            '.json', f'_SEMSEED_MUTATED_{num_of_locations_that_could_be_mutated}{file_extention}')
                        out_file_path = os.path.join(out_dir, out_file_name)
                        if fs.pathExists(out_file_path):
                            out_file_path = out_file_path.replace(
                                file_extention,
                                f'_{str(random.randint(0, 10000))}_{["a", "b", "c", "d"][random.randint(0, 3)]}{file_extention}')
                        try:
                            ...
                            with open(target_js_file_analysed['file_path'], 'r') as original_file:
                                with open(out_file_path, 'w') as mutated_file:
                                    for lineno, original_line in enumerate(original_file.readlines(), start=1):

                                        if lineno != target_range.start_line:
                                            # no modification on this line
                                            mutated_file.write(original_line)
                                            continue

                                        # modify line
                                        before_modification = original_line[:target_range.start_col - 1]
                                        after_modification = original_line[target_range.end_col - 1:]
                                        mutated_file.write(f'// MODIFICATION START\n')
                                        mutated_file.write(
                                        ...
                                        mutated_file.write(f'// MODIFICATION END\n')

                        except OverflowError as e:
                            tqdm.write(f'ERROR: Could not seed bugs to {file_name} because {e}')
                            bug_seeding.bug_metadata['error'] = str(e)
                        finally:
                            fs.writeJSONFile(data=bug_seeding.bug_metadata,
                                             file_path=out_file_path.replace(file_extention, '.json'))
                    else:
                        pass
