...
def get_only_idf_lit_containing_patterns(all_changes):
    ...
    We filter some of them here. For example:
        ...
        * we may select only those change patterns that has at least 'N' frequency
    """
    ...
    for t in all_changes:
        ...
        if 'Idf_' in ' '.join(t['fix']) or \
            'Idf_' in ' '.join(t['buggy']) or \
                'Lit_' in ' '.join(t['fix']) or \
                'Lit_' in ' '.join(t['buggy']):
